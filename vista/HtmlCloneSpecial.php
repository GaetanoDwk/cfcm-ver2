...
/**
 * Class HtmlCloneSpecial
 * Gestisce le operazioni di visualizzazione dei cloni degli host special
 */
class HtmlCloneSpecial
{

    /**
     * Visualizza i cloni di un host special
     * @param $Cloni
     */
    public function HTML_clone_host_special($Cloni)
    {
        echo"<br><br>
        ...
        foreach ($Cloni as $row)
        {
            ...
            echo"
                ...
                <form action='index.php'  method='post' target='_blank' style='display: inline;'>
                    ...
                    <button class='button_operazioni' type='submit' name='comando' value='edit_clone_special' style='border:none;'><i class='fa fa-pencil-square-o fa-2x'></i></button>
                    <!--Bottone con script JS che chiede conferma-->
                    <button class='button_operazioni' onclick='return confirm(\"Sicuro di voler eliminare questo elemento?\")' type='submit' name='comando' value='delete_clone_special' style='border:hidden;'><i class='fa fa-trash fa-2x'></i></button>
        ...
        <form action='index.php' method='post'>
                        <button type='submit' name='comando' value='add_clone_host_special' title='Aggiungi un Clone' style='border:none'><i class=\"fa fa-plus fa-2x\" aria-hidden=\"true\"></i></button>
                    </form>
      </div>";
    }
    ...
    /**
     * La funzione visualizza la pagina di aggiunta di un nuovo clone di un host special
     */
    public function HTML_add_clone_host_special()
    {
        echo"
            ...
            if($_SESSION['cli_type'] == 'P') {
                echo "
                    <form action='index.php' method='post' style='display: inline;'>
                        <button name='comando' value='view_host_special' style='position: absolute; left: 0%; border: none;' title='Torna ad Host Speciale'><i class='fa fa-chevron-circle-left fa-2x' aria-hidden='true'></i></button>
        ...
        echo"
            ...
            <center>
                    ...
                    <h6 class='docs-header'>INSERIMENTO NUOVO CLONE</h6></center>
            ...
            <script>
                ...
                <form action='index.php' method='post' enctype=\"multipart/form-data\">
                    ...
                    <input type='text' class='form-control' id='clo_stracq' name='clo_stracq' style='width:50%' placeholder='Strumento Acqusizione'><br>
                    <input type='text' class='form-control' pattern='.{32,32}' id='clo_md5' name='clo_md5' style='width:80%' placeholder='Md5 (32 caratteri)'><br>
                    ...
                    <textarea id='clo_log' name='clo_log' style='width: 100%; height: 35%;' placeholder='Incolla il Log'></textarea><br>
                    <button type='submit' name='comando' value='insert_clone_host_special' style='height: auto;'>Salva</button>
                    </center>
                </form>
        </div>";
    }
    ...
    /**
     * Visualizza la pagina di modifica delle info di un clone di un host special
     * @param int $id
     ...
     * @param string $log
     */
    public function HTML_edit_clone_special($id, $tipoacq, $altro, $stracq, $md5, $sha1, $sha256, $log)
    {
        echo"
        <div class='container'><br>";
            if(isset($_SESSION["post_ho_id"]))
            ...
            <center>
                <form action='index.php' method='post' enctype=\"multipart/form-data\">
                    <input type='hidden' class='form-control' id='clo_id' name='clo_id' style='width:70%' value=$id><br>
                        ...
                        <button type='submit' name='comando' value='update_clone_special' color='black' style='height: auto;'>Salva</button>
                </form>
            </center>
        </div>";
    }
